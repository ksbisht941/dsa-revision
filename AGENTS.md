...
- **ALWAYS** update the `README.md` file whenever you set up a playground. Ensure the `README.md` is professional, appealing, informative, and well-structured per guidelines. It should include an index table with columns "Problem Title" (with a hyperlink to the LeetCode problem), "Topic", "Status", and "View" (with a hyperlink to the local playground file in the repository).
- **ALWAYS** update the `README.md` index table whenever the user commands you to commit. Before committing, you must run the verification script (e.g., `verify_status.py`) or manually go through each modified/added file, run their tests, and update their `Status` column in the index table to reflect their current state (e.g., `✅ Solved`, `🚧 Attempting`, `❌ Not Attempted`).
